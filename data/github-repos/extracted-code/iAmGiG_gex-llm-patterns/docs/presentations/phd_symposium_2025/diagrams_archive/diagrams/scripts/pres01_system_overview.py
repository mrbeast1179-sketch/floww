...
def generate_layered_architecture():
    ...
    dot = graphviz.Digraph(comment="Complete System Architecture", format="png", engine="dot")
    ...
    with dot.subgraph(name="cluster_sources") as c:
        c.attr(label="Data Sources", fontsize="14", style="dashed", color="gray")
    ...
    with dot.subgraph(name="cluster_processing") as c:
        c.attr(label="Data Processing", fontsize="14", style="dashed", color="gray")
    ...
    with dot.subgraph(name="cluster_llm") as c:
        c.attr(label="LLM Agent", fontsize="14", style="dashed", color="#7D3C98")
    ...
    with dot.subgraph(name="cluster_validation") as c:
        c.attr(label="Validation Pipeline", fontsize="14", style="dashed", color="gray")
    ...
    dot.edge("obfuscate", "llm_reason", label="Obfuscated\ndata", fontsize="10")

    # LLM internal
    dot.edge("llm_tool", "llm_framework", label="Function\ncalls", fontsize="10", style="dashed")
    dot.edge("llm_reason", "llm_framework", label="Pattern\nanalysis", fontsize="10", style="dashed")

    # LLM to validation
    dot.edge("llm_framework", "outcome", label="Pattern detection\n+ Predictions", fontsize="10")
...
def generate_compact_flow():
    ...
    dot = graphviz.Digraph(comment="Compact System Flow", format="png", engine="dot")
    ...
    dot.attr(
        "node", fontname="Arial Bold", fontsize="13", style="filled,rounded", shape="box", height="0.9", width="2.5"
    )
    ...
    # Flow edges
    dot.edge("source", "process", penwidth="4", color="#34495E", arrowsize="1.5")
    dot.edge("process", "llm", penwidth="4", color="#34495E", arrowsize="1.5")
    dot.edge("llm", "validate", penwidth="4", color="#34495E", arrowsize="1.5")
    dot.edge("validate", "results", penwidth="4", color="#34495E", arrowsize="1.5")
...
def generate_detailed_with_swim_lanes():
    ...
    dot = graphviz.Digraph(comment="System Architecture - Swim Lanes", format="png", engine="dot")
    ...
    with dot.subgraph(name="cluster_data") as c:
        c.attr(label="Data Layer", fontsize="14", style="filled", color="lightgray", fillcolor="#F8F9F9")

        c.node("api_src", "Alpha Vantage\nAPI", fillcolor="#AED6F1", shape="cylinder")
        c.node("cache_db", "Cache DB\n(SQLite)", fillcolor="#AED6F1", shape="cylinder")
        c.node("fetch_data", "Data Fetcher\n\n1. Check cache\n2. API if needed", fillcolor="#D6EAF8")

    # ============ SWIM LANE 2: PROCESSING LAYER ============
    with dot.subgraph(name="cluster_proc") as c:
        c.attr(label="Processing Layer", fontsize="14", style="filled", color="lightgray", fillcolor="#FEF9E7")
    ...
    with dot.subgraph(name="cluster_ai") as c:
        c.attr(label="AI Layer", fontsize="14", style="filled", color="lightgray", fillcolor="#F4ECF7")

        c.node("llm_agent", "LLM Agent\n\nGPT-4o-mini\n+ o3-mini", fillcolor="#E8DAEF")
    ...
    with dot.subgraph(name="cluster_val") as c:
        c.attr(label="Validation Layer", fontsize="14", style="filled", color="lightgray", fillcolor="#E8F8F5")

        c.node("calc_outcome", "Outcome\nCalculator", fillcolor="#D5F4E6")
        c.node("stat_valid", "Statistical\nValidator", fillcolor="#D5F4E6")
        c.node("results_out", "Results\n\n71.5% Det\n91.2% Acc", fillcolor="#ABEBC6", shape="note")

    # ============ DATA FLOW EDGES ============
    dot.edge("api_src", "fetch_data", label="API call")
    dot.edge("cache_db", "fetch_data", label="check", style="dashed")
    dot.edge("fetch_data", "cache_db", label="store", style="dashed", dir="back")

    dot.edge("fetch_data", "gex_calc", label="options data", lhead="cluster_proc")
    dot.edge("gex_calc", "obfuscate_data", label="GEX")

    dot.edge("obfuscate_data", "llm_agent", label="obfuscated", lhead="cluster_ai")
    dot.edge("llm_agent", "pattern_detect", label="analysis")

    dot.edge("pattern_detect", "calc_outcome", label="predictions", lhead="cluster_val")
