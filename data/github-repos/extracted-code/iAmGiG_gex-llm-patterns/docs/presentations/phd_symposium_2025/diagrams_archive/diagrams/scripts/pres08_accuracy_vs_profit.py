...
ax1.set_ylabel("Predictive Accuracy (%)", fontsize=24, fontweight="bold", color=color_accuracy)
ax1.plot(
    quarters,
    accuracy_rates,
    "o-",
    ...
    linewidth=6,
    markersize=20,
    label="Predictive Accuracy (improving)",
    zorder=3,
)
...
ax1.annotate(
    ...
    xy=(3, 96.8),
    xytext=(0, 84.9),
    arrowprops=dict(arrowstyle="->,head_width=0.8,head_length=0.6", color=color_accuracy, linewidth=5, alpha=0.2),
)
ax1.text(
    1.5,
    90,
...
ax2.set_ylabel("Net Alpha (basis points)", fontsize=24, fontweight="bold", color=color_alpha)
ax2.plot(
    quarters, net_alpha, "s-", color=color_alpha, linewidth=6, markersize=20, label="Net Alpha (declining)", zorder=3
)
...
ax2.annotate(
    ...
    xy=(3, -0.5),
    xytext=(0, 20.8),
    arrowprops=dict(arrowstyle="->,head_width=0.8,head_length=0.6", color=color_alpha, linewidth=5, alpha=0.2),
)
ax2.text(
    2.5,
    6,
...
fig.text(
    ...
    fontsize=17,
    fontweight="bold",
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#ffe6e6", edgecolor="#dc3545", linewidth=3),
)
...
fig.text(
    ...
    fontsize=17,
    fontweight="bold",
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#fff8e6", edgecolor="#FFA500", linewidth=3),
)
...
fig.text(
    ...
    fontsize=17,
    fontweight="bold",
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#e6f7ed", edgecolor="#28a745", linewidth=3),
)
...
ax1.legend(
    lines1 + lines2, labels1 + labels2, loc="lower right", fontsize=18, frameon=True, edgecolor="#000000", fancybox=True
)
