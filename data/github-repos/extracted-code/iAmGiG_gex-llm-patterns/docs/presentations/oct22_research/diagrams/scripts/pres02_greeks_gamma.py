...
# Main title
fig.text(0.5, 0.95, 'Options Risk Metrics: The "Greeks"',
         ha='center', fontsize=32, fontweight='bold')

# Subtitle
fig.text(0.5, 0.90, 'Two metrics you need to understand',
         ha='center', fontsize=22, color='#003366', style='italic')

# ============ LEFT SIDE: DELTA ============
left_x = 4

# Delta header box
delta_header = FancyBboxPatch((1, 7.5), 6, 0.6,
                              boxstyle="round,pad=0.1",
                              facecolor='#E8F4F8',
                              edgecolor='#003366',
                              linewidth=4)
ax.add_patch(delta_header)
ax.text(left_x, 7.70, 'Delta (Δ): Stock Equivalence',
        ha='center', fontsize=24, fontweight='bold', color='#003366')

# Delta explanation box
delta_box = FancyBboxPatch((1, 4.80), 6, 2.2,
                           boxstyle="round,pad=0.1",
                           facecolor='#f5f5f5',
                           edgecolor='#000000',
                           linewidth=3)
...
ax.text(left_x, 6.7, 'HOW MUCH exposure you have',
        ha='center', fontsize=20, fontweight='bold', color='#000000')

ax.text(left_x, 6.1, 'Range: 0 to 1.0 for calls',
        ha='center', fontsize=18, color='#333333')

# Example box
example_box = FancyBboxPatch((1.1, 5.3), 5.75, 0.6,
                             boxstyle="round,pad=0.05",
                             facecolor='#fff',
                             edgecolor='#003366',
                             linewidth=2)
ax.add_patch(example_box)
ax.text(left_x, 5.6, 'Example: Δ = 0.50 = owning 50 shares',
...
# Gamma header box
gamma_header = FancyBboxPatch((9, 7.5), 6, 0.6,
                              boxstyle="round,pad=0.1",
                              facecolor='#FFE5B4',
                              edgecolor='#8B0000',
                              linewidth=4)
...
gamma_box = FancyBboxPatch((9, 4.8), 6, 2.2,
                           boxstyle="round,pad=0.1",
                           facecolor='#f5f5f5',
                           edgecolor='#000000',
                           linewidth=3)
...
ax.text(right_x, 6.7, 'HOW FAST exposure changes',
        ha='center', fontsize=20, fontweight='bold', color='#000000')

ax.text(right_x, 6.1, 'High gamma = URGENT rehedging',
        ha='center', fontsize=18, color='#8B0000', fontweight='bold')

# Example box
example_box2 = FancyBboxPatch((9.3, 5.3), 5.4, 0.6,
                              boxstyle="round,pad=0.05",
                              facecolor='#fff',
                              edgecolor='#8B0000',
                              linewidth=2)
ax.add_patch(example_box2)
ax.text(right_x, 5.6, 'Stock moves $1 → Δ changes by Γ',
        ha='center', fontsize=18, fontweight='bold',
...
# Title
ax.text(8, 4.2, 'Gamma Urgency Scale',
        ha='center', fontsize=24, fontweight='bold', color='#000000')
...
for x, level, desc, color, intensity in urgency_levels:
    ...
    circle = Circle((x, urgency_y), 0.4 * intensity,
                    facecolor=color, edgecolor='#000000', linewidth=3, alpha=0.7)
    ax.add_patch(circle)

    # Level label
    ax.text(x, urgency_y, level,
            ha='center', va='center',
            fontsize=18, fontweight='bold', color='#000000')

    # Description
    ax.text(x, urgency_y - 1.0, desc,
            ha='center', va='center',
...
for i in range(len(urgency_levels) - 1):
    ...
    arrow = FancyArrowPatch((x1, urgency_y), (x2, urgency_y),
