...
| #30 silent gate | `06b7502` | open, clean/mergeable | 2 | REWORK: no CI wiring; malformed/missing roots false-green |
...
The candidate AST scanner correctly returns 1 for a pass-only exception. A fresh
architect probe at `06b7502` also showed malformed Python returns 0 and a nonexistent
root returns 0. The PR changes only its script and two tests; `.github/workflows/lint.yml`
still runs the older grep gate. It is useful source material, not the completed P1
contract.
