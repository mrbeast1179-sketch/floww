...
# deterministic policy step (work→15min, idle→60min, 3rd idle/blocked→pause).
# Backoff/pause live in .gsd/scheduled_tasks.lock — the plist interval stays
# 900s and early wakes exit quietly. Overlap guarded by /tmp lockdir.
# Test plumbing without running a pass: GSD_DRY_RUN=1 GSD_STATE_FILE=/tmp/x.json
set -u

REPO=/Users/nav/Documents/GitHub/floww
PLAYBOOK=/Users/nav/.agents/skills/gsd-loop-build/playbook.md
LINKAGE=/Users/nav/.agents/skills/gsd-loop-build/scripts/ensure-linkage.mjs
STATE_FILE="${GSD_STATE_FILE:-$REPO/.gsd/scheduled_tasks.lock}"
LOG=/tmp/gsd-loop-build.log
LOCKDIR=/tmp/gsd-loop-build.lock
...
st() { "$PY" -c 'import json,sys; print(json.load(open(sys.argv[1])).get(sys.argv[2], sys.argv[3]))' "$STATE_FILE" "$1" "$2" 2>/dev/null; }
...
# --- preflight: runner + auth present? (missing = blocked per playbook) ---
EVENT=""; REASON=""
if [ ! -x "$OPENCODE" ]; then EVENT=blocked; REASON="opencode-missing";
elif ! command -v gh >/dev/null 2>&1; then EVENT=blocked; REASON="gh-missing";
elif ! gh auth status >/dev/null 2>&1; then EVENT=blocked; REASON="gh-auth";
...
if [ -z "$EVENT" ]; then
  ...
  if [ "${GSD_DRY_RUN:-0}" = "1" ]; then
    ...
    # Least-privilege unattended perms: repo-forbidden git ops + interactive
    # question tool denied (--auto approves the rest). See opencode docs.
    export OPENCODE_PERMISSION='{"question":"deny","bash":{"*":"allow","*--force*":"deny","*--force-with-lease*":"deny","*reset --hard*":"deny","*rebase*":"deny","*clean -fd*":"deny","*--no-verify*":"deny"}}'
...
d.update({"idle_count":int(i),"last_run":int(ts),"paused":(pa=="True"),"interval_minutes":int(n)})
