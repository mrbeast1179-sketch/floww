...
export default function OptionsChainTable({ ticker, spot }) {
  ...
  return (
    <div className="panel p-2">
      ...
      {/* Table */}
      <div className="overflow-auto max-h-96 text-[9px]">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {filtered.slice(0, 200).map((r, i) => {
              ...
              return (
                <tr key={i} className={`${nearSpot ? "bg-slate-700/30" : ""} hover:bg-slate-700/20`}>
                  <td className={`px-1 py-0.5 font-bold ${isCall ? "text-teal-400" : "text-purple-400"}`}>
                    {isCall ? "C" : "P"}
                  </td>
        ...
        {filtered.length > 200 && <div className="text-[9px] text-slate-500 p-1">Showing 200 of {filtered.length} rows</div>}
      </div>
    </div>
  );
}
