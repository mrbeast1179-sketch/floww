...
export function MorningBriefing({ ticker, spot }) {
  ...
  if (loading && !data) return <div className="panel p-3 text-slate-500 text-xs">Loading morning briefing…</div>;
  if (!data) return null;
  ...
  return (
    <div className="panel p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-[9px] text-slate-500">{new Date(data.asof).toLocaleTimeString()}</div>
      </div>
      ...
      <div className={`p-2 rounded border ${regimeBg}`}>
        ...
        <div className="text-[10px] text-slate-400 mt-0.5">{data.strategy?.regime_description}</div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
          <div className="flex justify-between">
            ...
            <span className={`mono font-bold ${data.key_levels?.dist_to_flip < 0 ? "text-rose-400" : "text-emerald-400"}`}>
              {data.key_levels?.gamma_flip ? fmt(data.key_levels.gamma_flip, 0) : "—"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">vs Flip</span>
            <span className={`mono ${data.key_levels?.dist_to_flip < 0 ? "text-rose-400" : "text-emerald-400"}`}>
              {data.key_levels?.dist_to_flip > 0 ? "+" : ""}{data.key_levels?.dist_to_flip} pts
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Call Wall</span>
            <span className="mono text-rose-400">{data.key_levels?.call_wall ? fmt(data.key_levels.call_wall, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Put Wall</span>
            <span className="mono text-emerald-400">{data.key_levels?.put_wall ? fmt(data.key_levels.put_wall, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Max Pain</span>
            <span className="mono text-amber-400">{data.key_levels?.max_pain ? fmt(data.key_levels.max_pain, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Spot</span>
            <span className="mono text-slate-200">{spot ? fmt(spot, 2) : "—"}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="text-[10px] text-slate-400 bg-slate-800/50 rounded px-2 py-1 border border-slate-700/50">
          {data.strategy?.position_sizing_note}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
          <div className="flex justify-between">
            ...
            <span className="mono text-rose-400">{data.risk_management?.resistance?.R1 ? fmt(data.risk_management.resistance.R1, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">S1</span>
            <span className="mono text-emerald-400">{data.risk_management?.support?.S1 ? fmt(data.risk_management.support.S1, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">R2</span>
            <span className="mono text-rose-400">{data.risk_management?.resistance?.R2 ? fmt(data.risk_management.resistance.R2, 0) : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">S2</span>
            <span className="mono text-emerald-400">{data.risk_management?.support?.S2 ? fmt(data.risk_management.support.S2, 0) : "—"}</span>
          </div>
        </div>
        {data.risk_management?.stop_suggestion?.below_put_wall && (
