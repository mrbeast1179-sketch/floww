...
def test_performance_stats_requires_auth(client):
    # Route is @router.get("/api/performance/stats") with prefix="/api" → /api/api/performance/stats
    r = client.get("/api/api/performance/stats")
    assert r.status_code == 401, f"expected 401, got {r.status_code}"


def test_performance_stats_succeeds_with_key(client):
    r = client.get("/api/api/performance/stats", headers={"X-API-Key": "test-key-h12"})
