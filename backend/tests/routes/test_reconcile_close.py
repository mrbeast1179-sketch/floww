...
def _seed(ticker, **over):
    base = {
        "ticker": ticker, "type": "call", "action": "buy", "strike": 500.0,
...
def _filled(order_id="ord-1", px=12.5, status="filled"):
...
class TestReconcilePendingClose:
    ...
    async def test_open_order_stays_pending(self, jeng):
        ...
        client = _FakeClient({"ord-2": _filled("ord-2", px=0, status="new")})
    ...
    async def test_canceled_order_never_closes(self, jeng):
        ...
        client = _FakeClient({"ord-3": _filled("ord-3", px=9.0, status="canceled")})
