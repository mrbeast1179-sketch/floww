...
class Portfolio:
    ...
    def remove_position(self, index: int):
        if 0 <= index < len(self.positions):
            ...
            self.cash += pos.entry_price * abs(pos.quantity) * 100 * sign

    def aggregate_greeks(self, spot: float, iv: float) -> Dict[str, float]:
        """Aggregate Greeks across all positions."""
        totals = {"delta": 0, "gamma": 0, "vega": 0, "theta": 0, "vanna": 0, "charm": 0, "vomma": 0, "zomma": 0}
    ...
    def scenario_analysis(self, spot: float, iv: float,
                           spot_shocks: List[float] = None,
                           vol_shocks: List[float] = None,
                           days_forward: int = 0) -> List[Dict[str, Any]]:
        """Run scenario analysis: shock spot, vol, and time."""
        ...
        for shock in spot_shocks:
            ...
            greeks = self.aggregate_greeks(new_spot, iv)
            pnl = self.total_pnl(new_spot, iv)
            scenarios.append({
                "type": "spot",
                "shock_pct": shock * 100,
        ...
        for shock in vol_shocks:
            ...
            greeks = self.aggregate_greeks(spot, new_iv)
            pnl = self.total_pnl(spot, new_iv)
            scenarios.append({
                "type": "vol",
                "shock_pct": shock * 100,
                "iv": round(new_iv, 4),
        ...
        for days in [1, 3, 5, 7, 14, 21]:
            if days <= days_forward:
                continue
            greeks = self.aggregate_greeks(spot, iv)  # Greeks already account for T
            pnl = self.total_pnl(spot, iv)
            scenarios.append({
                "type": "time",
                "days": days,
                "pnl": pnl["total_pnl"],
    ...
    def greek_neutral_hedge(self, spot: float, iv: float,
                             hedge_options: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        Calculate Greek-neutral hedge using matrix inversion.
        From Dynamic-Derivatives-Portfolio-Hedging (Bottacini).

        Given current portfolio Greeks and a list of available hedge options,
        find the quantities needed to neutralize gamma and vega, then delta.

        hedge_options: list of {"strike": float, "expiry": str, "type": str, "iv": float}
        """
        if len(hedge_options) < 2:
            return {"error": "Need at least 2 hedge options for gamma/vega neutralization"}

        current = self.aggregate_greeks(spot, iv)

        # Target: neutralize gamma and vega
        target_gamma = -current["gamma"]
        target_vega = -current["vega"]

        # Build Greeks matrix for hedge options
        option_greeks = []
        for opt in hedge_options[:2]:  # Use first 2 options
            T = max((datetime.strptime(opt["expiry"], "%Y-%m-%d").date() - date.today()).days / 365.0, 0.001)
            K = opt["strike"]
            sigma = opt.get("iv", iv)
            S = spot
            ...
            delta = bs_delta(S, K, T, sigma, 0, opt.get("type", "call"))

            option_greeks.append({
                "gamma": gamma * 100,  # per contract
                "vega": vega * 100,
                "delta": delta * 100,
                "strike": K,
                "expiry": opt["expiry"],
        ...
        # Matrix equation: [gamma1, gamma2] [w1] = [target_gamma]
        #                  [vega1,  vega2] [w2]   [target_vega]
        ...
        # Calculate new delta after gamma/vega hedge
        new_delta = current["delta"] + w1 * option_greeks[0]["delta"] + w2 * option_greeks[1]["delta"]
        # Delta hedge with underlying (delta of stock = 1 per share)
        stock_hedge = -new_delta  # shares to buy/sell
