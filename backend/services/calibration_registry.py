...
Status "claimed": the code states a calibration basis but no test pins it;
these are the calibration work queue — see unpinned().
"""
...
_BANDS: list[dict[str, Any]] = [
    {
        ...
        "test_path": "tests/test_amihud_illiquidity.py",
    },
    {
        "signal": "kyle_lambda",
        "parameter": "bands",
        ...
        "test_path": "tests/test_kyle_lambda.py",
    },
    {
        "signal": "composite",
        "parameter": "weights",
        ...
        "basis": "5-component split mirrored in composite_confidence; "
                 "parity guard written but in review, not yet on main.",
        "status": "claimed",
        "code_ref": "services/composite_flow_score.py:179",
        "test_path": None,  # guard in review, not on main
    },
    {
        "signal": "momentum",
    ...
    },
    {
        ...
        "test_path": "tests/test_alert_engine_volume_spike.py",
    },
    {
        "signal": "vex_scale",
        "parameter": "convention_factor",
        "value": "spot^2 * 0.01",
        "basis": "Exact factor between the GEX-parity display scale and the "
                 "per-unit-sigma scale pinned by a guard that is in review, "
                 "not yet on main.",
        "status": "claimed",
...
def unpinned() -> list[dict[str, Any]]:
    ...
    return [b for b in _BANDS if b["status"] != "pinned"]
